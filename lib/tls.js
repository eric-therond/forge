...
tls.handleServerHello = function(c, record, length) {
  ...
  if(c.fail) {
	...
	var renegotiation_info_ext = false;
  ...
  instead of continuing; see Section 4.1 for discussion.
	*/
	c.secure_renegotiation = false;
	
	/* 
	When a ServerHello is received, the client MUST check if it
  includes the "renegotiation_info" extension:
	*/
  var exts = msg.extensions;
  for(var i = 0; i < exts.length; i ++) {
	  if(msg.extensions[i].type[0] === 0xff && msg.extensions[i].type[1] === 0x01) {
			
			renegotiation_info_ext = true;
		  ...
		  if(!c.isConnected) {
					...
					*/
					...
					if(msg.extensions[i].data.getBytes() !== '\u0000') {
						// abort the handshake
						return c.error(c, {
							message: 'secure_renegotiation not supported by server.',
							send: true,
							alert: {
								level: tls.Alert.Level.fatal,
								description: tls.Alert.Description.handshake_failure
					...
					abort the handshake.
					*/
					if(c.secure_renegotiation && msg.extensions[i].data.getBytes() !== c.client_verify_data)
					{
						return c.error(c, {
							message: 'secure_renegotiation client_verify_data incorrect.',
							send: true,
							alert: {
								level: tls.Alert.Level.fatal,
								description: tls.Alert.Description.handshake_failure
  ...
  client MUST abort the handshake.
	*/
	if(c.secure_renegotiation && !renegotiation_info_ext) {
		return c.error(c, {
			message: 'secure_renegotiation not supported by server.',
			send: true,
			alert: {
...
tls.handleClientHello = function(c, record, length) {
  ...
  for(var i = 0; i < exts.length; i ++) {
	  if(msg.extensions[i].type[0] === 0xff && msg.extensions[i].type[1] === 0x01) {
		  ...
		  if(!c.isConnected) {
				/*
				The server MUST check if the "renegotiation_info" extension is
				...
				*/
				...
				if(msg.extensions[i].data.getBytes() !== '\u0000') {
					...
					return c.error(c, {
						message: 'secure_renegotiation not supported by client.',
						send: true,
						alert: {
							level: tls.Alert.Level.fatal,
							description: tls.Alert.Description.handshake_failure
  ...
  c.session.clientHelloVersion = msg.version;
	...
	// en cas de renegotiation = on utilise les paramètres courants
	if(!c.isConnected) {
		c.session.sp = {};
	}
  ...
  try {
    // decrypt 48-byte pre-master secret
		if(!c.isConnected) {
			...
			var sp = c.session.sp_renegotiated;
		}
...
tls.handleFinished = function(c, record, length) {
  ...
  client_verify_data and server_verify_data values for future use.
	*/
	if(!client) {
		// stored in the server connection
		c.client_verify_data = save_b.getBytes();
	} else {
		/*
		When the handshake has completed, the client needs to save the
		client_verify_data and server_verify_data values for future use.
		*/
		// stored in the client connection
		c.server_verify_data = save_b.getBytes();
	}
  ...
  if(type in hsTable[c.entity][c.expect]) {
    ...
    if(c.entity === tls.ConnectionEnd.server && !c.open && !c.fail) {
      ...
      c.session = {
		...
		if(c.entity === tls.ConnectionEnd.server && c.isConnected && type === tls.HandshakeType.client_hello) {
			// update hash for the server in case of renegotiation
			c.session.md5 = forge.md.md5.create();
			c.session.sha1 = forge.md.sha1.create();
		}
...
/*CAD*/[__,H7,__,__,__,__,__,__,__,__,__,__,__,__,__,__,__,__,__,__,__],
...
tls.createClientHello = function(c) {
  ...
  */
  ...
  if(!c.isConnected) {
    ...
    RECOMMENDED.
		*/
		// "renegotiation_info" (with extension type 0xff01) extension type is 2 bytes
		ext.putByte(0xff);
		ext.putByte(0x01);
		
		ext.putByte(0x00); // length
		ext.putByte(0x01); // length	
		ext.putByte(0x00); // renegotiated_connection field of Renegotiation_Info
    ...
    MUST NOT be included.
		*/
		...
		ext.putByte(0x00); // length
		ext.putByte(0x0d); // length 13
		ext.putByte(0x0c); // length 12
    
		ext.putBytes(c.client_verify_data);
		console.dir(c.client_verify_data);
		extensions.putBuffer(ext);
...
tls.createServerHello = function(c) {
  ...
  if(!c.isConnected) {
		...
		rval.putBytes(c.session.sp_renegotiated.server_random); // random time + bytes
  }
  ...
  rval.putByte(c.session.compressionMethod);
	...
	/*
	If the secure_renegotiation flag is set to TRUE, the server MUST
...
tls.createFinished = function(c) {
  ...
  var b_copy = b.copy();
	if(client) {
		// stored in the client connection
		c.client_verify_data = b_copy.getBytes();
	} else {
		// stored in the server connection
		c.server_verify_data = b_copy.getBytes();
	}
...
tls.createConnection = function(options) {
  ...
  var c = {
    ...
    heartbeatReceived: options.heartbeatReceived,
		secure_renegotiation: false, // flag, indicating whether secure renegotiation is in use for this connection
		client_verify_data: null, // the verify_data from the Finished message sent by the client on the immediately previous handshake
		server_verify_data: null, // the verify_data from the Finished message sent by the server on the immediately previous handshake
